...
static const int64_t MIN_ANON_SERVICE_COIN = 100000 * COIN;	
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %sDOPE %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
int64 CWallet::GetMintedBalance() const
{
    int64 nTotal = 0;
    {
        LOCK(cs_wallet);
...
//		nCombineThreshold = GetProofOfWorkReward(pIndex0->nHeight, MIN_TX_FEE, pIndex0->pprev->GetBlockHash(), pIndex0->nTime) * 10;
    ...
    // Calculate coin age reward
    {
        ...
        nCredit += GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, pIndex0->nHeight+1);
    }
